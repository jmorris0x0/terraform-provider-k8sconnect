...
- `disable_webhooks` (Boolean) Prevent Helm from running validation webhooks. Defaults to false.
...
- `recreate_pods` (Boolean) Perform pod restart during upgrade/rollback. Defaults to false.
...
- `set_string` (Attributes List) Set string values (forces value to be treated as string). (see [below for nested schema](#nestedatt--set_string))
...
- `values` (String, Sensitive) Values in YAML format to merge with chart defaults. Typically used with file() function.
- `version` (String) Chart version. Can include digest for OCI charts (e.g., '1.0.0@sha256:...'). If not specified, uses the latest version.
...
- `manifest` (String, Sensitive) Computed rendered manifest (multi-document YAML).
