...
- `force_conflicts` (Boolean) Force ownership of fields currently managed by other controllers during Server-Side Apply. When false (default), apply fails if another field manager owns a field, protecting against accidental takeovers. When true, forcibly claims ownership of conflicting fields. Useful when intentionally taking control from controllers like HPA or when migrating resources from other Terraform states.
