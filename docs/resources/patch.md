...
- `managed_fields` (Map of String) Tracks which field manager owns each field path in the patched resource. Shows 'k8sconnect' for fields managed by this provider, or external manager names (e.g., 'kubectl', 'hpa-controller') for files managed by other systems. When ownership changes appear in diffs, it indicates another system has taken control of those fields.
