...
k8sinline solves this elegantly:
...
```hcl
resource "k8sinline_manifest" "app1" {
...
resource "k8sinline_manifest" "app2" {
...
- **k8sinline**: Currently requires inline config, limiting adoption
...
# With k8sinline: Just works!
resource "google_container_cluster" "primary" {
  name = "prod"
}

resource "k8sinline_manifest" "ingress_controller" {
...
resource "k8sinline_manifest" "bootstrap" {
...
# After (k8sinline with provider auth)
provider "k8sinline" {
  kubeconfig_file = "~/.kube/config"
  context         = "prod"
}
...
resource "k8sinline_manifest" "new_app" {
...
resource "k8sinline_manifest" "existing_app" {
  yaml_body = file("app.yaml")
