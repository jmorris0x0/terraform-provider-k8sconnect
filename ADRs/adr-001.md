...
Use Kubernetes dry-run to show accurate diffs. Store a JSON projection of managed fields. Basic tree operations handle the field extraction. Simple and effective.# Architecture Decision Record: Kubernetes Terraform Provider with Managed Fields Projection
...
Use Kubernetes dry-run to show accurate diffs. Store a JSON projection of managed fields. Basic tree operations handle the field extraction. Simple and effective.
...
Implement a provider that uses **Managed Fields Projection** to achieve accurate structured diffs. This approach is simpler than it initially appears - the core logic is basic tree operations that any competent developer can implement.
...
The implementation is straightforward - just two key operations:
...
### Negative
- **None significant**: The apparent complexity is mostly standard tree operations
...
### Critical Components
1. **Field Path Extraction**: Simple tree traversal to identify all field paths (~10 lines)
2. **Projection Logic**: Basic get/set operations on paths (~20 lines)
3. **Type Preservation**: Handle lists, maps, primitives correctly
4. **Dry-Run Integration**: Single API call with proper error handling
...
The "sophisticated" field operations are just basic tree manipulation:
```go
// Extract paths - simple recursion
...
The solution is elegantly simple: instead of trying to predict what Kubernetes will do (impossible), we ask Kubernetes what it will do (dry-run), then show only the parts we care about (projection).
