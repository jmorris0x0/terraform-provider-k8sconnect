// internal/k8sconnect/resource/manifest/interpolation_test.go
...
// TestAccManifestResource_UnknownInterpolation tests that yaml_body handles
// interpolations that reference computed/unknown values from other resources
func TestAccManifestResource_UnknownInterpolation(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.source", "id"),
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.dependent", "id"),
...
resource "k8sconnect_manifest" "source" {
  yaml_body = <<YAML
apiVersion: v1
kind: ConfigMap
metadata:
...
resource "k8sconnect_manifest" "dependent" {
  yaml_body = <<YAML
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  ...
  source_id: "${k8sconnect_manifest.source.id}"
...
YAML
  ...
  depends_on = [k8sconnect_manifest.source]
}
`, namespace, namespace, namespace)
}

// TestAccManifestResource_ChainedInterpolation tests multiple levels of interpolation
func TestAccManifestResource_ChainedInterpolation(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.first", "id"),
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.second", "id"),
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.third", "id"),
...
metadata:
  ...
  namespace: %s
data:
  first_id: "${k8sconnect_manifest.first.id}"
YAML
  ...
  depends_on = [k8sconnect_manifest.first]
}

resource "k8sconnect_manifest" "third" {
  yaml_body = <<YAML
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  first_id: "${k8sconnect_manifest.first.id}"
  second_id: "${k8sconnect_manifest.second.id}"
YAML
  ...
  depends_on = [k8sconnect_manifest.second]
}
