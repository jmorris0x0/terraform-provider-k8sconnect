// internal/k8sconnect/resource/manifest/cluster_scoped_test.go
...
// TestAccManifestResource_ClusterScopedWithInvalidNamespace tests that cluster-scoped
...
func TestAccManifestResource_ClusterScopedWithInvalidNamespace(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.test_sa", "id"),
					...
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.test_crb", "id"),
					...
					resource.TestCheckResourceAttrSet("k8sconnect_manifest.test_crb", "managed_state_projection.%"),
				),
			},
		},
...
resource "k8sconnect_manifest" "test_sa" {
  yaml_body = <<YAML
apiVersion: v1
kind: ServiceAccount
metadata:
...
resource "k8sconnect_manifest" "test_crb" {
  yaml_body = <<YAML
...
YAML
  ...
  depends_on = [k8sconnect_manifest.test_sa]
}
