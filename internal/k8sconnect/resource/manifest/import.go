// internal/k8sconnect/resource/manifest/import.go
package manifest
		...
		if homeDir == "" {
			resp.Diagnostics.AddError(
				...
				"KUBECONFIG environment variable is not set and HOME directory could not be determined.\n\n"+
					...
					"  terraform import k8sconnect_manifest.example \"prod/default/Pod/nginx\"",
			)
			return
		}
	...
	if _, err := os.Stat(kubeconfigPath); os.IsNotExist(err) {
		resp.Diagnostics.AddError(
			...
			fmt.Sprintf("Kubeconfig file not found at: %s\n\n"+
				...
				"  terraform import k8sconnect_manifest.example \"prod/default/Pod/nginx\"", kubeconfigPath),
		)
		return
	}
