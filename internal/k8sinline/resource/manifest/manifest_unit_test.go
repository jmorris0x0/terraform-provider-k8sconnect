...
func TestGetDeleteTimeout(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "default namespace timeout",
...
metadata:
  name: test-pod`,
			deleteTimeout:  types.StringValue("15m"),
...
metadata:
  name: test-pod`,
			deleteTimeout:  types.StringValue("0"),
			expectedResult: 0,
		},
		{
			name: "pvc timeout",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data := manifestResourceModel{
				YAMLBody:      types.StringValue(tt.yamlBody),
				DeleteTimeout: tt.deleteTimeout,
			}

			result := r.getDeleteTimeout(data)
...
func TestWaitForDeletion(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupClient: func() k8sclient.K8sClient {
				...
				// First call returns object, second returns NotFound
				client.GetError = errors.NewNotFound(schema.GroupResource{Resource: "pods"}, "test")
				return client
			},
		...
		{
			name: "timeout without ignore finalizers",
			setupClient: func() k8sclient.K8sClient {
				client := k8sclient.NewStubK8sClient()
				// Always return an object (never deleted)
			...
			expectError:       true,
			expectErrorString: "timeout after",
		},
		{
			name: "timeout with ignore finalizers",
			setupClient: func() k8sclient.K8sClient {
				client := k8sclient.NewStubK8sClient()
				// Always return an object (never deleted)
				client.GetResponse = &unstructured.Unstructured{
			...
			expectError:      false, // Should not error when ignoring finalizers
		},
		{
			name: "skip waiting with timeout 0",
			setupClient: func() k8sclient.K8sClient {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.waitForDeletion(ctx, client, gvr, obj, tt.timeout, tt.ignoreFinalizers)

			if tt.expectError {
				if err == nil {
					t.Error("expected error but got none")
				} else if tt.expectErrorString != "" && !strings.Contains(err.Error(), tt.expectErrorString) {
					t.Errorf("expected error containing %q, got %q", tt.expectErrorString, err.Error())
				}
			} else {
				if err != nil {
					t.Errorf("unexpected error: %v", err)
...
// Integration test with acceptance test framework
func TestAccManifestResource_FinalizerHandling(t *testing.T) {
	// This would be in the acceptance test file
	const testConfig = `
variable "raw" { type = string }
provider "k8sinline" {}

resource "k8sinline_manifest" "test_finalizer" {
  yaml_body = <<YAML
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: test-pvc-finalizer
  namespace: default
spec:
...
YAML
  ...
  delete_timeout = "30s"  # Short timeout for testing
...
`
	...
	// Test would verify:
	// 1. PVC gets created with finalizers
	// 2. On destroy, provider waits for finalizers to be resolved
	// 3. If timeout occurs, provides helpful error message
	// 4. User can adjust timeout as needed
}

// Performance test for waiting mechanism
func TestWaitForDeletionPerformance(t *testing.T) {
	r := &manifestResource{}

	// Test that the polling doesn't consume excessive resources
	client := k8sclient.NewStubK8sClient()
	client.GetError = errors.NewNotFound(schema.GroupResource{Resource: "pods"}, "test")
	...
	start := time.Now()
	err := r.waitForDeletion(ctx, client, gvr, obj, 5*time.Second, false)
	...
	// Should complete quickly since object is immediately "not found"
	if elapsed > 1*time.Second {
		t.Errorf("deletion wait took too long: %v", elapsed)
	}

	// Verify we didn't make excessive API calls
	if len(client.GetCalls) > 5 {
		t.Errorf("made too many API calls: %d", len(client.GetCalls))
	}
}
